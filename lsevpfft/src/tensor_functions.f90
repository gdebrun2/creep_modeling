...
contains
    ...
    subroutine lu_inverse(a, n)
        ...
        if (info /= 0) then
            write (*, '(A,I0)') 'Singular matrix in lu_inverse, info=', info
            stop 1
        end if
        ...
        if (info /= 0) then
            write (*, '(A,I0)') 'Failed inverse in lu_inverse, info=', info
            stop 1
        end if
    ...
    subroutine lu_eqsystem(a, b, n)
        ...
        if (info /= 0) then
            write (*, '(A,I0)') 'Failed system solve in lu_eqsystem, info=', info
            stop 1
        end if
    ...
    subroutine polar_dcmp(F, R, U)
        ...
        if (info /= 0) then
            write (*, '(A,I0)') 'Error in LAPACK dgesvd, info=', info
            stop 1
        end if
    ...
    pure function skew33(matrix) result(skew)
        implicit none
        double precision, intent(in) :: matrix(3, 3)
        double precision :: skew(3, 3)

        skew = 0.5d0 * (matrix - transpose(matrix))
    end function skew33

    pure function tr33(matrix) result(trace)
        implicit none
        double precision, intent(in) :: matrix(3, 3)
